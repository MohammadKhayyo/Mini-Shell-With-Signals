...
pid_t child_For_control_z; // This global pid saves the child pid when the ctrl z is clicked
pid_t child_No_pipe; // help for save son pid
...
void ProcessForNonePipes(char ** Command, int * How_Many_process, int * HowManyPipes, int countOfWords); //this method will take the Shell command and activate it  if it did not have Pipes, and it receives the signal order for stopping and continuing the process by entering legal input.
    ...
    if (strcmp(FirstCommand[0], "cd") == 0 || strcmp(SecondCommand[0], "cd") == 0 || strcmp(ThirdCommand[0], "cd") == 0) { //if user enter cd
        ...
        if (BeforePipe == -1) {
            ...
            if (AfterPipe == -1) {
                ...
                if (AfterSecondPipe == -1) {
                    /* error occurred */
                    perror("folk failed");
                    FreeMallocForTwoPipes(FirstCommand, SecondCommand, ThirdCommand, theFirstArray, theSecondArray, theThirdArray, countOfWords, countOfWords2, countOfWords3);
                    FreeIfFailed(How_Many_process, HowManyPipes);
                } else if (AfterSecondPipe == 0) {
                    /* son process */
...
void closePiPes(int pipe_fd[2], int pipe_fd2[2], char ** FirstCommand, char ** SecondCommand, char ** ThirdCommand, char * theFirstArray, char * theSecondArray, char * theThirdArray, int * How_Many_process, int * HowManyPipes, int countOfWords, int countOfWords2, int countOfWords3) { //method that closes the open file descriptor entries
    if (close(pipe_fd[0]) == -1 || close(pipe_fd[1]) == -1 || close(pipe_fd2[0]) == -1 || close(pipe_fd2[1]) == -1) {
        /* error occurred */
        perror("close field");
        FreeMallocForTwoPipes(FirstCommand, SecondCommand, ThirdCommand, theFirstArray, theSecondArray, theThirdArray, countOfWords, countOfWords2, countOfWords3);
        FreeIfFailed(How_Many_process, HowManyPipes);
...
void ProcessForNonePipes(char ** Command, int * How_Many_process, int * HowManyPipes, int countOfWords) { //this method will take the Shell command and activate it  if it did not have Pipes, and it receives the signal order for stopping and continuing the process by entering legal input.
    int Status;
    if (strcmp(Command[0], "fg") == 0 && countOfWords == 1) { // in these condition if user write fg will send signal to continue
        kill(child_For_control_z, SIGCONT); // send signal (SIGCONT) to the Process that has Pid = child_For_control_z
        waitpid(child_For_control_z, & Status, WUNTRACED); // this wait Releases the son's resources  if there are a change of situation has occurred
        if (WIFSTOPPED(Status)) { // check if there are stopped signal
            printf("\n");
        }
        freeDynamicAllocation(Command, countOfWords);
    } else {
        child_No_pipe = fork(); /* fork another process */
        if (child_No_pipe < 0) {
            /* error occurred */
            perror("folk filed");
            exit(EXIT_FAILURE);
        } else if (child_No_pipe == 0) {
            /* child process */
            ...
            /* parent process */
            waitpid(child_No_pipe, & Status, WUNTRACED); // this wait Releases the son's resources  if there are a change of situation has occurred
            if (WIFSTOPPED(Status)) { // check if there are stopped signal
                printf("\n");
                child_For_control_z = child_No_pipe; // save the child pid
            }
            freeDynamicAllocation(Command, countOfWords);
        }
    }
